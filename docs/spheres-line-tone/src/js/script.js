...
async function init() {
  ...
  spheres = _.map(_.range(num), i => {
    const beginX = -margin * col / 2;
    const beginY = margin * row / 2;
    const colIndex = i % row;
    const rowIndex = (i / col) | 0;
    const sphere = new THREE.Mesh(geometry, material);
    sphere.position.copy(new THREE.Vector3(
      beginX + colIndex * margin + margin / 2,
      beginY - rowIndex * margin - margin / 2,
      0,
    ));
    scene.add(sphere);
    return sphere;
